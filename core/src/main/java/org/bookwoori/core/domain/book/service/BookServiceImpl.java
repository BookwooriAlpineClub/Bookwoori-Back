...
import org.bookwoori.core.domain.book.infrastructure.BookJpaRepository;
...
public class BookServiceImpl {
    ...
    public BookEntity getOrCreateBookByIsbn(String isbn) {
        Optional<BookEntity> existingBook = bookRepository.findByIsbn13(isbn);
        if (existingBook.isPresent()) {
            return existingBook.get();
        }
        BookEntity newBook = getBookByIsbn(isbn).toEntity();
